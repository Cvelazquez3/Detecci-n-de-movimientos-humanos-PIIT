...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function processIMUData(data, isLive = true) {
        ...
        // Calibración de sesgo en reposo (siempre ~3 segundos reales,
        // independientemente de si la Fs activa es 10Hz o 50Hz)
        if (biasCalibrating && isLive) {
            biasSamples.push({x: gx, y: gy, z: gz});
            const muestrasNecesarias = Math.round(activeSampleRateHz * 3);
            if (biasSamples.length >= muestrasNecesarias) {
